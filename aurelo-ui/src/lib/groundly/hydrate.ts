import type { FrameworkKey, FrameworkResult, GroundlyReport } from "./types";

type Point = { text: string; citation_index: number | null };
...
function cite(text: string): number | null {
...
function pointsFrom(block: string): Point[] {
  const chunks = block
    .split(/\n+/)
...
function extractCategories(text: string, keys: string[]): Record<string, Point[]> {
  ...
  const hits: { key: string; index: number; length: number }[] = [];
...
function detectBcg(text: string) {
...
function detectAnsoff(text: string) {
  const lower = text.toLowerCase();
...
export function hydrateFrameworkResult(key: string, result: FrameworkResult): FrameworkResult {
  if (!result) return result;
  const next: FrameworkResult = { ...result };
  ...
  if (cats) {
    ...
    if (field && !(next as Record<string, unknown>)[field]) {
      (next as Record<string, unknown>)[field] = extractCategories(text, cats);
    }
  }
...
export function hydrateReport(report: GroundlyReport): GroundlyReport {
  if (!report?.results) return report;
  const results: GroundlyReport["results"] = {};
  for (const [key, value] of Object.entries(report.results)) {
    results[key] = hydrateFrameworkResult(key as FrameworkKey, value);
  }
